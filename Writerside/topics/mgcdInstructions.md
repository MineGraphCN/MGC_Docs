...
**粗体**
: 表示**强调**或**关键词**，或在已有斜体时的 ***人／团队名*** 。

*斜体*
: 表示 *引用* 或 *人／团队名* 或 **_加粗时二次强调_**。也配合 _<format color="Gray">灰色文字</format>_ 作图片及表格下的注释。

[跳转](#约定 "跳转链接描述")
: 跳转到站内其他位置，或当链接后跟有 ↗ 时，表示跳转 [站外链接](https://b23.tv/BV1GJ411x7h7 "好康的") 。将鼠标悬停在其上有时会有和缩写一样的提示。
...
`代码`
: 带有背景的等宽字体。表示这是程序运行时显示/日志/代码内容，或计算机行为。代码中内容的约定：
...
<path>路径</path>
: 无背景等宽字体。表示文件路径，如 <path>X:\Minecraft\.minecraft\resourcepacks\\</path>。

$\text{LaTeX}$ 公式
: 形如 $y = ax^2 + bx + c$ 的衬线西文字体，在部分设备上公式中的中文可能显示为非衬线字体，由于其本身构建时会转换为矢量图，因此这些内容中的西文均无法复制。
