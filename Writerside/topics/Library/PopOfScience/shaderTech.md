...
我们使用当时正在开发中的 iterationT 3.2 作为示例，它在新版本中大幅优化了全局光照，在不考虑光照遮挡的情况下足以产生极其优异的光照效果。
...
自 iterationT 0.16.1（2.0.0 之后不久的开发版）开始，它的 RSM 算法一直都有问题 ^<b>1</b>^ ，直到 iterationT Beta 3.1.07 才修复。
...
使用 SEUS Renewed 作为 RSM 示例，SEUS LPVGI 作为 LPV 示例。
...
使用 SEUS Renewed 和 iterationT 3.0 作为 RSM 示例，SEUS LPVGI 作为 LPV 示例，Sundial 作为光线追踪示例。
...
原版平滑光照在方块接触区域周围的顶点上添加的明暗变化。受 `平滑光照级别` 控制。
...
图片来自 _Tahnass_，光影为当时正在开发中的 iterationT 3.2。
<tabs>
<tab title="无">
...
我们使用 Complementary 光影作为对比示例，它同时支持 TAA 和 FXAA。
...
**<tooltip term="DLSS">深度学习超级采样</tooltip>**，由 *英伟达* 开发的一种升采样技术。通过降低分辨率并调用显卡 ^NVIDIA^ 的<tooltip term="TCore">**张量核心**</tooltip>来猜测原始分辨率下该处像素的内容。
...
我们使用带有 TAAU 的 Kappa 来作为示例，比较使用 TAAU 和原生低分辨率的画质差异。
...
在 Java 版中，我们主要使用 [路径追踪](terms.md#路径追踪 "使用蒙特卡洛随机采样并模拟光子多次反弹，经由漫反射、镜面反射、折射等直到达到退出条件所最终产生的光照。") 。在此之前的光影，考虑到 Minecraft 游戏过程的动态性和一些算法限制，没办法像其他游戏一样使用预烘焙等类似技术来改善光栅化渲染。例如光源仅能按照原版的发光机制来单色发光；反射面完全无法反射屏幕外的物体（天空除外，光影通常会手动处理它们，因此能拿到它们的全部数据或绘制函数）等。  
...
> 
> 在 _Cody Darr_ [最近（并非）的日志](https://www.patreon.com/posts/creative-and-84935136 "Patreon，可能需要代理")中提到，PTGI 的下一个版本将改为使用稀疏数据结构存储场景信息，这样不仅能突破体素距离限制，同时还存储实体信息。但是这篇文章距今已过去两年，而 PTGI 的下一个版本仍是遥遥无期。
> 
> Rethinking Voxels 利用 Iris 提供的自定义大尺寸缓冲区保存了相对高精度的符号距离场而不是体素，虽然不能获取实体的纹理信息，但是可以将场景中的几何信息悉数保存，配合上 Iris 的另一个独占特性 <tooltip term="SSBO">SSBO</tooltip> 来存储光源数据，可以兼容（几何上的）几乎任何模组。
...
在上文中，我们已经展示了光线追踪如何影响全局光照，这里我们再给出一些光线追踪的优势示例。传统光栅化光影使用 iterationT 3.2 开发版，光线追踪光影则使用 Sundial。
- 反射视野外的物体
  <tabs group="ss_rt">
