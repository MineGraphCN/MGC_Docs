...
> 根据它所计算的对象，我们也将其称为**像素着色器**（Pixel Shader）。也有些地方称它为片段着色器，这听起来会让人有些困惑，因此我们现在都使用和“图元”对应的“片元”。
</def>
...
> 在现代 GPU 中，有一种名为**提前深度测试**（Early Depth Testing）的功能，可以在片段着色器执行之前在 GPU 上预先判断场景的几何遮挡情况，在着色器开始处理之前先丢弃将会或已经被遮挡的像素。
