...
光影通常以多个着色器组成，着色器接收渲染模组提供的各种变量，以及程序之前乃至上一帧计算好的存入缓冲区的信息，按照程序进行计算后输出到指定的缓冲区。  
...
<def title="细分着色器" id="ts">

**Tessellation Shader**，由细分控制着色器（**T**essellation **C**ontrol **S**hader，可选）和细分评估着色器（**T**essellation **E**valuation **S**hader）两部分组成。
- TCS 负责生成额外控制点 ^**1**^ 和确定细分级别以避免意外的扭曲；TES 负责根据控制点计算细分顶点来让几何体有更多细节。
- TCS 的计算对象为每个面片（Patch），每个面片由一组控制点定义，作为一个待细分的模板；TES 的计算对象为 TCS 结束之后由硬件细分器生成的每个细分顶点。

虽然细分着色器是在 GL 4.0 时就提供的可选阶段，但只有 Iris 支持。

**[1]** 顶点着色器传出的顶点在细分着色器中被称为控制点，细分评估着色器只能修改新生成的细分顶点而不能修改控制点。
</def>
<def title="几何着色器" id="gs">

**Geometry Shader**，这个阶段是**可选的**，它的主要职责是根据图元生成新顶点，可以通过特定的索引值确定需要处理的顶点。
> 图元通常为点（一个顶点）、线条（两个及以上顶点 ^线框渲染^ ）或多边形（三个及以上顶点），也包括细分着色器专用的面片。
</def>
<def title="片元着色器" id="fs">

**Fragment Shader**，它的主要职责是处理像素的颜色，也是大多数效果程序所处的位置，计算对象是图元覆盖的每个片元（通常是像素）。
> 根据它所计算的对象，我们也将其称为**像素着色器**（Pixel Shader）。也有些地方称它为“片段”着色器，这听起来会让人有些困惑，因此我们现在都使用和“图元”对应的“片元”。
> 
...
**Compute Shader**，这个阶段是**可选的**，它负责进行抽象计算。可以任意存取缓冲区，但是不能传入自定义变量，也没有默认输出。它可以以 [](terms.md#图像){summary=""} 类型作为媒介间接输出数据。
...
> 
> ![热插拔游戏卡带](famicom_glitch.webp "热插拔游戏卡带"){border-effect="rounded" width="384"}
> 
...
然而随着场景几何体增多、几何体之间相对于视角的遮挡越来越频繁，光照算法也日渐复杂，这种做法开始产生越来越多不必要的开销。因为每个片元着色器都会将所有传入图元覆盖的所有像素计算一遍，即使像素内容会被更靠前的图元遮挡。

> 在现代 GPU 中，有一种名为**提前深度测试**（Early Depth Testing）的功能，可以在片元着色器执行之前在 GPU 上预先判断场景的几何遮挡情况，在着色器开始处理之前先丢弃将会或已经被遮挡的像素。
...
延迟渲染还提供了一个优势，在向前渲染法中，像素着色器仅会对图元覆盖区域进行着色，这就导致了与其他片元有交互的特效几乎无法绘制（比如镜头特效、模糊和 [环境光遮蔽](terms.md#ao){summary=""} 等），而延迟渲染法由于使用了铺屏四边形，可以取到屏幕中任何位置的信息，因此绘制它们不在话下。
...
> 延迟渲染也不完全优于向前渲染，一些特殊材料只有在向前渲染下才能表现出应有的质地；由于延迟渲染使用多个屏幕尺寸的缓冲区，对显存的带宽要求较高，在移动设备上其性能也可能不如向前渲染；而且在半透明处理上延迟渲染也会更加复杂，因为缓冲区通常只能保存一层几何信息，需要创作者在处理时进行斟酌，更多时候更是会在渲染半透明时混用向前渲染。
> 
