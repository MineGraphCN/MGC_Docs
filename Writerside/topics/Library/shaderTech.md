...
[环境光遮蔽](terms.md#ao)（**Ambient Occlusion**，简称 **AO**）是一种广泛用于现代游戏渲染的图形技术，其算力消耗相对较少的同时能带来非常大的画面观感改进，其模拟了现实世界中光线在物体的凹角和接缝中产生阴影的现象。
...
> 目前在 Java 版的光影中主要使用 SSAO，也是游戏界的环境光遮蔽技术中相对使用最多的。
...
**Ray Tracing AO**，意为**路径追踪环境光遮蔽**。通过在世界空间光线追踪计算 AO，效果最好的同时性能消耗也最高。
...
一般来说，[全局光照](terms.md#gi)（**Global Illumination**，简称 **GI**）=直接光照+间接光照。全局光照，表现了直接照明和间接照明的综合效果。它有多种实现方法，例如辐照度、光线追踪、光子贴图、光照探针等。当光从光源被发射出来后，碰到障碍物就反射和折射，经过无数次的反射和折射，物体表面和角落都会有光感，像真实的自然光。
