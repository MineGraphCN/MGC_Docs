...
1. 检查 **GPU 占用** ：
   ...
   - 若发现核显运行满载而独立显卡空闲，则需在系统设置中启用高性能电源计划，或 [检查你的 GPU 驱动和 Windows 设置是否正确](ts_common.md#selGPU "通用问题 - 选择运行 GPU") 。
...
- **光影包读取出现问题**
  ...
  - 若同时伴随着左下角的**一大串报错**，同时在切换光影界面的底部看到显卡型号为 Intel 核心显卡（非 Arc 系列的显卡），同时如果你**确认**你的电脑有独立显卡，则可能说明你没有正确配置显卡。参考 [通用问题 - 独立显卡没有正确启用](ts_common.md#独立显卡没有正确启用) 解决。
...
<seealso>
  <category ref="related">
    ...
    <a href="Mods_compatibility.md" summary="这篇文章列出了我们所知的模组和光影间兼容性冲突的解决办法。">光影和模组兼容性</a>
  </category>
</seealso>
