...
调试线框（区块边界框）和拴绳都在 `basic` 中处理，不需要在片段着色器中进行纹理采样，因此我们的顶点着色器不必传出 UV。
...
线框的片段着色器处理比较麻烦，因为它们实际上是半透明的，但是又在第一轮几何缓冲中。如果处理不当很容易导致后处理效果覆盖在线框上，或者将线框的数据覆盖掉场景的数据，从而造成视觉瑕疵。在这里先提供多种方案处理它们：
- 作为世界中的几何体绘制
