...
在上一章中，我们完成了一个几何缓冲并在延迟处理中渲染了动态阴影，但是仅有 Terrain 还不足以构成一个完整的场景。

从本章开始，我们将会从固体几何缓冲出发，逐步构建完整的场景，并且添加一些咋咋呼呼看起来很酷的特效。当然，实现各种效果就少不了原理的轰炸，其实大多数原理都很简单，解释也很详尽，所以希望大家能尽力而为。

本节我们将会逐个编写第一轮几何缓冲（固体几何缓冲）并输出到多个缓冲区中，然后修改延迟处理程序来妥善处理它们。只要将不同批次几何的同类型数据写入同一个缓冲区，延迟渲染程序就会自动根据这些数据进行处理。

</tldr>

## 程序分类

OptiFine 将整个场景被拆分到了将近 20 个几何缓冲，要为它们逐一编写处理程序会给今后的修改维护带来很多不必要的麻烦。
...
#endif
```
将其放入一个新的文件夹 `shaders\programs\` ，这里的内容都是可以直接使用的着色器程序。最后在各几何缓冲的顶点和片元阶段调用对应的内容：
```glsl
#version 330 core
...
> 如果两个阶段之间有相同 `#include` 文件，则可以将它们提到顶部：
> ```glsl
...
void main() {
    ...
    float front = sign(float(gl_FrontFacing) - 0.5);
    normal = fs_in.normal * front * 0.5 + 0.5;
...
*/
```
我们将之前的法线缓冲区设置为了 `RGB16_SNORM` ，`_SNORM` 后缀表示保存的内容会保留符号进行归一化，因此在几何缓冲中我们不必将法线映射到 $[0,1]$ 然后在延迟处理时将其又映射回 $[-1,1]$ 了，并且我们使用了 16 位的数据存储，提高了不少数据精度。

> 事实上，类似 `RGB8`、`RGB16` 这种三通道组合后位数不为 8 的偶数倍的像素格式在几乎所有硬件中也仍然按 `RGBA` 处理。因为将它们紧凑排布在内存中很麻烦，比如要对齐到 32 位，在安排完一个通道之后 8 位像素会空出一个额外通道，下一个像素会空出两个通道，以至于在一些扩展中根本没有这种修饰符。
> 
> 如果你想的话，也可以直接使用 `RGBA16_SNORM` 。
...
>
> 在我们的教程中不会刻意节省缓冲区通道，但是你可以通过习题 1 初步尝试将法线数据和光照数据保存在一起。这种保存方式比较“粗暴”，我们会在之后的章节讨论更加妥善的法线编码方式。
>
{style="note"}
...
```
然后在片元着色器中先处理纹理 Alpha 值，再乘上顶点 Alpha 值用以判定是否是半透明实体：
```glsl
[... main ...]
fragColor = texture(gtexture, fs_in.uv);
if(fragColor.a <= alphaTestRef) discard;
...
在结束之前，还有一个小细节需要我们处理——实体的受击效果和爆炸闪烁，这两个效果是不包含在顶点色彩中的。在原版里，我们需要一个额外的 UV 来采样一张上红下白的两像素纹理以确定实体的颜色，但是 OptiFine 为我们简化了这个操作。我们只需要在 `Uniforms.glsl` 中声明 `uniform vec4 entityColor;`，然后在之前的颜色输出处理结束后手动进行混合：
```glsl
...
摄像机在常规游戏模式下发光隐身实体始终会以完全不透明渲染，而观察者模式则会以半透明渲染，我们无法知道一个实体是否处于隐身状态，再加上发光实体先于普通实体渲染，如果按半透明写入则会遮挡之后的实体，因此这里就统一视为固体几何了：
```properties
...
调试线框（区块边界框）和拴绳都在 `basic` 中处理，不需要在片元着色器中进行纹理采样，因此我们的顶点着色器不必传出 UV。
...
线框的片元着色器处理比较麻烦，因为它们实际上是半透明的，但是又在第一轮几何缓冲中。如果处理不当很容易导致后处理效果覆盖在线框上，或者将线框的数据覆盖掉场景的数据，从而造成视觉瑕疵。在这里先提供多种方案处理它们：
- 作为世界中的几何体绘制
  1. 禁用混合，直接忽略不透明度，将它作为无遮蔽类那样处理。副作用是会参与到后处理中其他效果，视你的喜好而定。
  2. 禁用混合，将内容连带不透明度写入单独的缓冲区，在之后的延迟渲染中单独处理它们，并按需进行混合，这样就可以保留半透明效果。
- 作为 HUD 绘制
  1. 禁用混合，忽略不透明度，仅写入颜色缓冲区（0 号缓冲区），然后在后期所有效果都忽略这片区域，副作用是模糊和扭曲类的效果可能会采样到这片区域，需要手动判断。
  2. 和实体渲染的方案 2 类似，启用 RGB 通道的混合并禁用 Alpha 通道的混合，然后写入颜色缓冲区，然后在后期利用 Alpha 值来混合效果强度，副作用是线框覆盖范围背后的 AO 无法渲染，并且也会有方案 1 的限制。
  3. 禁用混合，将内容连带不透明度写入单独的缓冲区，然后在管线的最末端将它们混合到输出内容中。

在此我们选择最为简单的方案，即作为世界中的几何体绘制的方案 1。顶点着色器中的输出只有颜色：
```glsl
out VS_OUT {
    vec4 color;
} vs_out;
...
最后，判定顶点的 ID，如果是偶数则取反偏移量以便朝另一边扩展 ^**3**^。具体操作为对顶点 ID 取 2 的模，如果是偶数则值为 0，即需要反转偏移方向，否则为 1。则我们可以将其减 0.5 然后使用 `sign()` 函数取其正负号，就获得了乘数 -1 或 1。最后将偏移量加到顶点 NDC 上，再逆转透视除法 ^**4**^，就得到了最终的线框。
```glsl
...
**[5]** 虽然条带只有 4 个顶点，但是每相邻产生的三个顶点都会自动拼合一个三角形，这里使用了**几何着色器**来给每个拼合出来的三角形的内部顶点 ^**6**^ 染色。你可以自行尝试将线框宽度拉大，然后观察将顶点颜色更改为 `vec3(float(gl_VertexID % 4) / 3.0)` 和 `vec3(gl_VertexID % 2)` 之后的变化。  
...
如果你想在片元着色器中禁用深度写入，可以使用：
```glsl
float depth = texelFetch(depthtex0, ivec2(gl_FragCoord.xy), 0).r;
gl_FragDepth = max(depth, gl_FragCoord.z);
```
即当线框相比之前几何缓冲写入片元的更靠近摄像机时，我们将当前的深度设置为之前几何缓冲的深度（相当于不覆写深度信息），否则照常写入当前片元的 $z$ 坐标（照常进行深度测试，因为被遮挡，线框片元会被丢弃）。

线框的专用几何缓冲程序我们保存在 `gbuffers_line.glsl` 中。

最后的最后，别忘记欣赏欣赏我们处理完毕的场景！

![欢迎回来，固体几何！](soildGbuffer_finish.webp){width="700"}

## 习题

1. （可选）将法线通道和光照强度存入同一个缓冲区，法线的第三分量可以使用 $z = \sqrt{1-(x^2+y^2)} = \sqrt{1-\vec{a} \cdot \vec{a}}$ 进行重建，其中 $\vec{a}{(x,y)}$ 表示仅有两朝向的法线向量。设置纹理格式时应该以组合存储数据中值域范围大的内容为准。
2. 消化消化线框几何的相关内容，你也可以尝试自己修改 `VIEW_SHRINK` 和 `LineWidth` 值，以及尝试更改线框颜色。
3. 如果你仔细的话，可能会发现第二轮几何缓冲的雨雪和破坏粒子以及手持的半透明方块以一种非常鬼畜的形态回来了，一下雨就伸手不见五指。这是因为第二轮几何缓冲的大多数程序都会回退到 `textured` 上，而它本身又会回退到 `basic`，因此记得像我们之前那样新建一个 `gbuffers_textured.fsh` 然后丢弃所有片元！
