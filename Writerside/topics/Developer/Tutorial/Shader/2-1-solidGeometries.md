...
在上一章中，我们完成了一个几何缓冲并在延迟处理中渲染了动态阴影，但是还不足以构成一个哪怕完整的场景。

从本章开始，我们将会从固体几何缓冲出发，逐步构建一个完整可用的光影，并且添加一些咋咋呼呼看起来很酷的特效。当然，实现各种效果就少不了原理的轰炸，其实大多数原理都很简单，解释也很详尽，所以希望大家能尽力而为。

本节我们将会逐个还原第一轮几何缓冲（即固体几何缓冲）中的内容，并在延迟处理中妥善安排它们。
