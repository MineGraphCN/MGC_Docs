...
因为表面通常不会绝对光滑，因此轻微的偏移只是会造成片段产生的高光减弱，不会导致平面上只有一个点能看见光源虚像，换句话说，高光会漫开，漫开强度取决于我们的视线 ^**1**^ 靠近反射中的光源方向的程度以及表面的粗糙程度。说到两个向量的靠近程度，自然而然的，我们就能知道需要将高光方向与视线方向进行点乘。每个片段与我们摄像机的差值就是当前片段的视线方向，因此我们只需要求得当前片段相对摄像机的方向与反射目标方向的同向程度，就可以算出高光的强度了。为此，我们还需要知道当前片段的方向。
...
我们之前将材质信息取在了 `vec4 material` 中，现在，我们需要将材质的金属性，也就是 G 通道取出来以便处理 $F_0$。之前我们提过，在金属中，我们通常使用表面颜色来表示反射的颜色，那么对于现实中不存在的半金属来说，我们可以人为地认为 $F_0$ 表示纹素的材质倾向金属的比例。

表面颜色与反照率的微妙关系导致了前者在几乎任何物理渲染中都无法做到一视同仁，而且我们不希望在金属中浪费它们，因此表面颜色通常会被用于控制金属反射的强度：
- 在非金属材质中，表面颜色通常就用于反照率，这与物理相符；
- 在金属材质中，由于金属会吸收所有几乎散射光而只剩下镜面反射，虽然硬编码金属反射使用物理正确的 $F_0$，但硬编码本身总归是受限的，因此表面颜色也会在没有硬编码的材质中作为菲涅尔强度来补足细节（某种意义上就是反射的颜色）。
