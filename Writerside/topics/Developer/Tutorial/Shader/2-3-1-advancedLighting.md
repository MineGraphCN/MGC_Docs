...
这里使用了一个新的 GLSL 内建函数 `smoothstep(gtype a, gtype b, gtype x)` ，它根据 `x` 的值和过渡始末点 `a`、`b` 将映射后的值平滑处理到 $[0,1]$ 上。其内部实现为 $[3t^2-2t^3,\ t=\mathrm{Clamp}{(\frac{x-a}{b-a}, 0, 1)}]$，其中 $\frac{x-a}{b-a}$ 是很经典的重映射函数 `remap(a,b,x)`，会将 $(x,y)$ 映射到从 $(a,0)$ 到 $(b,1)$ 的连线上。然后将区间外的值裁切之后，再使用多项式进行平滑。
