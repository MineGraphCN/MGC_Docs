...
```
这里我们进行了一次 `clamp()` 是为了避免浮点除法精度问题导致法线与视线方向模长略微大于 1（1.000...01）且两者夹角极小时，点乘结果会大于 1，导致 $(1-\cos\theta)^5$ 出现负数（使用浮点指数还可能产生 <tooltip term="NaN">NaN</tooltip>），进而让最终的光照混合结果产生黑点。
...
如果你实在厌烦了 `pow(gvec, vec(float))`，可以像这样重载一串：
```glsl
