...
上一小节中，我们还留下了法线纹理中的 BA 通道，Alpha 通道的高度图通常会用作绘制视差，但是对我们这一章来说还太早，而余下的 Blue 通道中保存的环境光遮蔽信息，我们就很熟悉了。
...
为了纠正这一点，聪明的古人想出了在存储色彩 $C$ 时预先将原始色彩 $C_\text{Raw}$ 进行逆变换，即 $C=C_\text{Raw}^{1/(2.35\sim2.5)}$。这样，当存储色彩 $C$ 需要显示在屏幕上时，最终呈现的图像就会经过下式的变换：
$$
C_\text{CRT} = C^{2.35\sim2.5} = \left(C_\text{Raw}^{1/(2.35\sim2.5)}\right)^{2.35\sim2.5} = C_\text{Raw}
$$
...
```
这对于大多数常规颜色来说没有什么大不了，因为它们自始至终都在 sRGB 空间中运转。然而光照之类的物理量应当是在线性空间中的，由于我们没有进行手动处理，线性的光照被以 2.2 次幂输出了，也就是说我们目前所编写的画面实际上是光照不准确 ^**1**^ 的！

**[1]** 在我们目前的创作中这还不是什么大问题，因为我们手动控制了光照强度，使其保持在 $[0,1]$ 这个 SDR 亮度范围附近，顶多会让场景变暗一些。但是到了 HDR 渲染中，情况就完全不同了：由于幂次的存在，那些大于 1 的亮度会被轻易地进行翻倍，越亮的光照会带来越夸张的翻倍，最后导致整个世界的光照强度完全失控。因此进行伽马校正也是为今后的创作铺路。
...
3. GLSL 中没有内建的 `remap(a,b,x)`、`saturate(x)` 和 `remapSaturate(a,b,x)` 函数，因此你可以将它添加到你的 Utilities 里。注意 `remapSaturate(a,b,x)` 的内部实现为 `saturate(remap(a,b,x))`，不要写反了！
