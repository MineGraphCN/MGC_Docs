...
这是 [iterationRP](https://www.minegraph.cn/shaderpacks/54) 中所使用的八面体映射算法（谢谢你，Tahnass），我们就直接搬过来了。我们主要解析一下 `EncodeNormal()` 中的处理方法。

第一步中，它将球面上的坐标（长度向量绕空间所有点形成一个球面）映射到八面体表面上，八面体表面满足坐标分量和为 1 ^**1**^。实际上更准确的映射是要把整个向量 `n` 除以分量之和，然而我们并不需要 $z$ 坐标，因此省略了它。
...
同时，一些资源包的纹理环境光遮蔽可能过于微妙或强烈，因此还需要额外处理。需要注意，环境光遮蔽的强度和 `normalMap.b` 是负相关的，因此当 `normalMap.b == 1.0` 时，无论如何都应当没有 AO 强度。此外，我们还需要注意环境光遮蔽的强度提前达到最强（环境光照为 0）时不能再让其继续减小（否则环境光照为负），因此还需要一个钳制。
```glsl
[... Settings ...]
...
原本保存的线性亮度就完美还原了，这个将原始数据通过倒数次幂处理的过程就被称为**伽马校正**（Gamma Correction）。
