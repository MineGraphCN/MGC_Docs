...
> 你也可以发挥[想象力](https://b23.tv/BV13E411E77v)（和 Geogebra 之力、俺寻思之力 whatever）使用其他不改变 $x=0, y=0$ 和 $x=1, y=1$，并且会让曲线凸起（$\lim x\to0 ,\ \Delta y>\Delta x ;$  $\lim x\to1, \Delta x>\Delta y$）的函数，比如 $\sqrt[n]{|x|}$、$1−(|𝑥|−1)^n$ 等来改变权重的形态。根号型函数对中央区域的权重更加激进，而幂型函数则会牺牲极远处和近处的阴影来让中远处的阴影也保持不错的质量。
...
> 
> 另一种在单张阴影贴图上处理的方法叫**直线纹理形变**（**R**ectilinear **T**exture **W**raping）阴影，它会根据视口中阴影的占比动态调整它们在阴影空间中的比例，可以仅使用低分辨率阴影纹理就达到屏幕像素级别的阴影细节。

### 柔和阴影

虽然近处的阴影细节得到了细化，但是阴影边缘看起来却仍然很粗糙，有许多毛刺和锯齿，这也是阴影纹理与屏幕像素无法精确对齐所致。

虽然我们无法完全避免锯齿的瑕疵，但可以设法遮掩：将阴影的边缘进行柔化。如果我们在一个阴影纹素的周围进行多次取样来判断当前片段被遮蔽的量，就可以求到阴影边缘的过渡，这就是所谓**百分比渐进过滤**（**P**ercentage **C**loser **F**ilter）。
...
> 编者习惯只使用一层花括号来编写这种嵌套循环，这不是一个好习惯，初学者一定要规范语法使用。

现在，我们需要确定每次采样的位置，采样的基础朝向很简单，就是 `normalize(vec2(i,j))`，我们主要看每一步应该在方向上走多远。最首先的，当然我们需要以纹素为采样尺度，这里的步进坐标是索引坐标，因此我们要将它乘上阴影贴图的纹素大小：
```glsl
...
刚才的总步进 *方向* ^**2**^ 是基于未形变阴影屏幕空间的，而我们目前所使用的采样坐标还在 NDC 中，因此为确保正确的扩散，需要将步进长度进行对应的映射，也就是减半：
```glsl
...
现在，我们的光照优化已经接近尾声了，在结束这一节之前，让我们再来干一个比较轻松的事情。虽然说是比较轻松，但是由于网络上的各种文章互相摘抄，而作为源头之一的 Learn OpenGL 可以说是完全没解释清楚，导致大家在理解其原理时会出现很多混乱，本节将致力于用最易懂的文字解释这一过程。
