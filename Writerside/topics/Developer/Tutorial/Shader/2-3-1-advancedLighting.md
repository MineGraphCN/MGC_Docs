...
pow(max(dot(...)), smoothness) * 4.0 * pow(smoothness, 2.0);
```
...
布林-冯氏光照相比较于单纯的冯氏光照会更加明亮和平滑，因为半程向量与法线的夹角相对变化更慢，在球上的高光扭曲程度也会因此减弱，可以使用 4 ~ 5 倍的指数来勉强对齐冯氏光照的观感。
...
>
> 虽然在微观上，每个原子向各个方向散射的强度实际上只和光子与原子碰撞之后反弹的方向有关，一束均匀平行光打在球面法线上的散射结果又会呈现出光源所在半球较强而另一半球较弱（半球累积强度比 $1 : \sqrt{2}$）。因此每个原子针对直接来自光源的光子散射确实更集中在光源所在半球中，进入视口的散射光强度也就会随着视角的不同而移动，并且原子表面绝对光滑，只有一个点能接收到光线，就更像镜面高光了。
>
> 但是如果把视角放到更宏观的尺度上，由于多重散射，非均质材料中逸出的还可能是在内部多个原子上反弹了数次之后的光子，方向也由此变得随机。如果光子在材料内部散射时能量不衰减，那么最终表现出来向各个方向的漫反射强度是近似一致的。
>
...
>
> 事实上，反射也会受到光照角度的影响，因为入射光的总量会由于和漫反射相同的原因而变少，现实生活又告诉我们，虽然会减弱，但是反射光会在从掠角进行观察时看起来更加明显，我们马上就会在这一小节解释它们。
>
{title="小知识"}

回看我们写出来的高光效果，我们还没能解决所有表面看起来都像金属的问题。这是因为我们单纯地将镜面反射叠加到了本来的光照强度上，这些高光在非金属材质上不应该总是如此明显，反射出来的光线也不应该受表面颜色本身影响 ^**1**^。

**[1]** 非金属的反射光与表面碰撞之后就逸出了，它们几乎不与材质产生交互。再者固有色或者说反照率在物理上是不包含反射部分的，我们通常看到的颜色都由漫反射提供，而反射则专注于产生与光源色彩几乎相同的明亮高光（和反射场景），这在上面的小知识中已经讨论过了。

如果你仔细阅读了上面的小知识，或许会开始思考这个问题：既然几何表面接受到光照之后会产生漫反射和镜面反射，那在入射光强度一定时，漫反射和镜面反射的强度应该是此消彼长的，再考虑到材料内部散射被吸收的部分，总的出射光应该比入射光强度更弱。换句话说，漫反射和镜面反射的能量总和不应该大于入射光的能量，这就是**能量守恒**。
...
菲涅尔强度表征的是折射和反射的比例，而散射则是在非均质材质中的折射，而漫反射又来源于散射，因此菲涅尔方程可以用来计算漫反射和反射的比例。完整的菲涅尔公式相当复杂，对于 GPU 来说也相当昂贵，幸好 Schlick 提出了一种近似方法，将其简化为了：
...
对于普通的非金属而言，我们可以将 $F_0$ 统一设置为一个小值。在很多教程包括~~万恶之源~~ Learn OpenGL 中，都将其设置为了 0.04，这以你自己的喜好为准。
...
> 这里我们进行了一次 `clamp()` 是为了避免浮点除法精度问题导致法线与视线方向模长略微大于 1（1.00...01）且两者夹角极小时，点乘结果会大于 1，进而导致 $\vec{a}\cdot\vec{b}=\cos\theta>1 \rightarrow 1-\cos\theta < 0 \rightarrow (1-\cos\theta)^5 < 0$（使用浮点指数还可能产生 <tooltip term="NaN">NaN</tooltip>），让最终的光照混合结果产生黑点。
...
> 金属严格来说没有漫反射部分，由于现实生活中有来自各个方向的环境光照，因此没什么问题，但是我们目前还没有其它的光照来源，因此金属看起来可能会很奇怪（只有一个光斑，其它地方全黑）。不过我们马上就会在下一小节中为其添加环境光照，因此会将剔除金属中的漫反射留作习题。
...
litSceneAmbient *= AMBIENT_BRIGHTNESS * lightmap.t // 设置环境光照强度
                 * albedo.a * skyColor; // 上色和 AO

vec3 litSceneBlock = BLOCK_BRIGHTNESS * lightmap.s // 设置方块光照强度
                   * albedo.rgb; // 方块光照应用 AO 与否取决于你的喜好

vec3 litSceneBase = BASE_BRIGHTNESS * albedo.a
                  * albedo.rgb; // 无光环境基本亮度，乘上环境光遮蔽来确保纵深感
...
> GPU 中标量之间的乘法更省性能，因此可以先分别完成标量乘法和矢量乘法，最后再将它们乘在一起。

和天空光照这种万向光照不同，由于我们仅能依赖纹理上的方块光照强度，而无法知道方块光源的具体位置，但是它们又 _确实_ 有一个具体来源，这种复杂的情况下如果强行应用菲涅尔会让高光效果看起来很奇怪，因此我们将贡献全部归入漫反射。基础光照也是如此，它只是对画面的单纯提亮，因此也不参与光照模型的处理。
...
你可能会发现矿洞中的方块边缘会漏光，这是因为在矿洞中游戏会自动剔除地上的区块，方块的背面就暴露在了阴影深度纹理上，由于偏移，边缘就会产生一些漏光。我们可以假定天空光照强度为 0 的区域也不存在阳光，即只在天空光照不为 0 的区域计算光照：
```glsl
float lit = 0.0;
if(lightmap.t != 0.0) {
    lit = [...];
...
我们之前使用的法线是按顶点尺度记录的法线数据，如果想要表面有真实的凹凸光照效果，就需要增加许多顶点。然而，过多的顶点会给 CPU 和 GPU 都带来巨大的负担，因此，和从颜色纹理上获取额外的纹素颜色、从高光纹理上获取额外的材质属性类似，我们使用纹理来记录每个纹素的法线朝向。这样虽然不能形成物理遮蔽，但是可以通过改变片元朝向数据来影响受光效果，使其产生视觉凹凸，从而使用少量的顶点就能获得大量细节。这就是**法线映射**（Normal Mapping）。
...
上一小节提到过，OptiFine 会读取两种特殊后缀的纹理，也介绍了其中一种 `_s` 后缀的高光贴图。而这一小节的主角就是另一种特殊纹理，以 `_n` 为后缀的**法线纹理**（**N**ormal texture），我们在几何缓冲程序中声明 `uniform sampler2D normals;` 就可以访问它了。
...
我们的第一步是从 RG 两通道数据重建出完整的法线向量。由于法线本身只需要两个向量就可以重建（考虑到看不见背离场景的法线，我们只需要默认 $z$ 分量为正就好，就像顶点法线里我们干的那样），而且法线直接指向镜头时值就为 $(0,0,1)$（值域为 $[-1,1]^3$）因此 OptiFine 提供的默认纹理颜色值就是 $(127,127,255,255)$，所以不是什么大问题。
...
在之前的几何缓冲处理中，我们使用了矩阵来将顶点从局部坐标转换到裁切坐标，然后又使用法线矩阵来将法线从局部朝向转换到视口朝向。

表面法线也很类似，由于朝向应当基于顶点法线，因此要将它们转换到和顶点法线一样的空间，就需要一个以顶点法线为基准的坐标系作为中间人。当然，顶点法线只有一条，仅靠它是不够的。其他两个方向实际上也很容易敲定，那就是以 UV 朝向为基准的两个方向：它们不仅相互垂直，也与法线垂直，这样三个相互垂直的向量直接就构成了一个正交坐标系。

这两个以 UV 方向为基准的向量称为**切线**（Tangent）和**副切线**（Bitangent）^**1**^，而它们和法线构成的参考系所在的空间就被称为**切线空间**（Tangent Space）。法线贴图就在这个空间中绘制，只不过将值域从 NDC 的 $[-1,1]^3$ 变换到了位图的 $[0,255]^3$，然后又被 GL 读取解释为了 $[0,1]^3$。

因此，我们只需要将视口空间中法线、切线和副切线按顺序构造为一个旋转矩阵，即可作为我们寻找的中间人将切线空间的法线纹理变换到视口空间中了。
...
![切线空间](advancedLighting_tbn_space.webp){width="500"}
...
需要注意，叉乘是给定 $xy$ 平面的向量然后叉出 $z$ 轴，也在右手坐标系中（四指指向 $x+$，然后手掌面向 $y+$，大拇指伸直指向的即为 $z+$），因此参数是有序的，不可以调换两向量的顺序，否则计算出的向量会反向。游戏提供的切线（包括手性）也不一定是归一化的，叉积向量的长度又是平面两向量围成的平行四边形面积，因此**切线一定要手动归一化，叉乘（带手性）之后的向量也一定要记得归一化**！

这里求出来的副切线本身是在视口空间的（因为法线和切线都乘了 `normalMatrix`），因此就可以将它们构造为一个矩阵，以便将在切线空间中绘制的法线数据旋转到它们本该在的朝向上。由于这个矩阵按照切线（$x+$）、副切线（$y+$）、法线（$z+$）的顺序构造 ^**2**^，因此也被称之为 **TBN 矩阵**（**T**angent **B**itangent **N**ormal Matrix）。
```glsl
[... 顶点着色器 ...]
out VS_OUT {
    [...]
...
**[2]** 由于游戏中存在一些纹理会镜像和旋转的方块（比如命令方块），如果不随着镜像和旋转变换手性，UV 的翻转就会导致叉积翻转，从而导致副切线计算出错。当 UV 被镜像时，手性会被设置为负。此外，Minecraft 的纹理坐标会在 $y$ 轴上进行翻转，形成类似 DirectX 的左手系，因此你会发现叉乘时的 $xyz$ 轴（切线，法线，副切线，以及手性纠正）和构造 TBN 时的 $xyz$ 轴（切线，副切线，法线）不同。
...
表面法线变换到世界中之后，可能背离视线（与视口方向夹角小于 90°），可以进行简单的处理让它平缓回退到顶点法线上。要想知道表面朝向的背离视口的程度，我们需要知道摄像机的朝向，即摄像机与片元连线的方向。

因此，我们先在顶点着色器额外中输出顶点的坐标信息：
...
然后，我们在片元着色器中将方向进行归一化，然后根据方向的偏差程度来将过大的纹理法线平滑过渡到顶点法线。
```glsl
[... 片元着色器 ...]
in VS_OUT {
    [...]
...
这里使用了一个新的 GLSL 内建函数 `smoothstep(gtype a, gtype b, gtype x)` ，它根据 `x` 的值和过渡始末点 `a`、`b` 将映射后的值平滑处理到 $[0,1]$ 上。其内部实现为 $[3t^2-2t^3,\ t=\mathrm{Clamp}{(\frac{x-a}{b-a}, 0, 1)}]$，其中 $\frac{x-a}{b-a}$ 是很经典的重映射函数 `remap(a,b,x)`，会将 $(x,y)$ 映射到从 $(a,0)$ 到 $(b,1)$ 的连线上，并裁切区域外的值。使用 `remap()` 处理完毕之后，它会进一步使用多项式将数值进行平滑处理最后返回新值。
...
第一步中，它将球面上的坐标（单位长度向量绕空间所有点形成一个球面）映射到八面体表面上，八面体表面满足坐标分量和为 1 ^**1**^。实际上更准确的映射是要把整个向量 `n` 除以分量绝对值之和，然而我们并不需要 $z$ 坐标，因此省略了它。

**[1]** 你可以从二维推广，$|x|+|y|=1$ 是一个顶点在 $(0,\pm1)$ 和 $(\pm1,0)$ 四个点的正方形，因此 $|x|+|y|+|z|=1$ 就是一个顶点落在 $(\pm1,0,0)$、$(0,\pm1,0)$ 和 $(0,0,\pm1)$ 六个点上的正八面体。
...
我们将 `normalMap` 作为输出变量，将其写入 6 号（或者 7 号）缓冲区中，然后在需要用它们的时候读取并使用 `DecodeNormal()` 解码。
```glsl
[... 几何缓冲 ...]
/* DRAWBUFFERS:...6 */
...
> 
> 可以额外计算一个 `dot(lightDir, vertexNormal)` 替代。
> 
{style="note"}

为了保证大家不被绕晕，我们将上述过程总结为一个流程图，**每一个写入了顶点法线的几何缓冲都应该完整执行它们**，没有纹理的几何缓冲也应该将顶点法线编码并将余下两分量设置为 `vec2(1.0)`，然后覆写进缓冲区以便同步顶点法线和表面法线缓冲区对应位置上表示的对象。
...
flowchart TD
    ...
    out --> TBN1
    in --> CVS
    YES & NO --o SMOOTH{{"平滑插值"}} --> EW[/"编码到八面体合并余下
    法线纹理数据并输出"/]
...
上一小节中，我们还留下了法线纹理中的 BA 通道，Alpha 通道的高度图通常会用作绘制视差，我们暂且不看，而余下的 Blue 通道中保存的环境光遮蔽信息，我们就很熟悉了。

和顶点环境光遮蔽类似，我们只需要将纹理环境光遮蔽乘入环境光照中：
```glsl
...
同时，一些资源包的纹理环境光遮蔽可能过于微妙或强烈，因此还需要额外处理。需要注意，环境光遮蔽的强度和 `normalMap.b` 是负相关的（相当于它本身存的是环境光强度），因此当 `normalMap.b == 1.0` 时，无论如何都应当没有 AO。此外，我们还需要注意环境光遮蔽的强度提前达到最强（此时 AO 为 1，环境光照为 0）时不能再让其继续增强（否则环境光照为负），因此还需要一个钳制。
```glsl
[... Settings ...]
...
就算纹理环境光遮蔽过于微妙，你也能一眼看出表面法线带来的光照质量提升了！

![顶点法线光照 VS 表面法线光照](advancedLighting_snvsvn_lighting.webp){width="700"}

## 阴影优化

在第一章中，我们利用阴影贴图在场景中完成了阴影绘制，但是它们实在过于粗糙，如果在 2048x 的阴影深度纹理（约 16MB）上来绘制 8x8 的区块，它就已经顶不住了：

![好难看的影子！](advancedLighting_terrible_shadows.png){width="700"}

此外，你还会看到阴影有难看的方形边缘，这都是这一节我们将会处理的问题。

### 阴影权重

OptiFine 的阴影空间始终会以摄像机所在的方块为中心，而实际游戏中近景的占比和细节量通常也确实远景更多，这就给我们提供了第一个阴影优化思路：假定在阴影空间中心附近的场景更容易出现在视口空间，需要将资源倾向它们，据此改变远景和近景在阴影空间中的占比。

还记得吧，进行透视除法之后，空间坐标会落在 $[-1,1]$ 上，因此当阴影空间靠近摄像机所在原点时，我们可以用下式增大它在屏幕上的占比：
$$
x' = \frac{|x|}{\alpha|x|+1-\alpha} \cdot x_\mathrm{Sign} = \frac{x}{\mathrm{Mix}(1,|x|,\alpha)}
$$
其中，$x$ 表示原本空间所在的位置，分母项表示权重强度，混合比例 $\alpha \in [0,1)$：

- 当 $\alpha=0$ 时，分母为 1，$x'=x$，空间的位置不变；
- 当 $0<\alpha<1$ 时：
  - 当 $|x|$ 一定，随着 $\alpha$ 增大，分母逐渐靠近 $|x|$，结果会接近 1，因此小的原始坐标就会占用很大一部分映射后的坐标；
  - 当 $\alpha$ 一定，随着 $|x|$ 从 0 到 1，结果从 $\frac{0}{\mathrm{Mix}(1,0,\alpha)} = 0$ 变为 $\frac{\pm1}{\mathrm{Mix}(1,1,\alpha)} = \pm1$，因此最值不会改变。

我们不关心坐标绝对值大于 1 的情况，因此这个式子可以很好地处理 NDC 的权重。同样的，你可以在 [我们的 GeoGebra 演示](https://www.geogebra.org/calculator/dqaep8vv) 中尝试这个函数。

> 你也可以发挥 [想象力](https://b23.tv/BV13E411E77v)（和 Geogebra 之力、俺寻思之力 whatever）使用其他不改变 $x=0, y=0$ 和 $x=1, y=1$，并且会让曲线凸起（$x\to0 ,\ \Delta y>\Delta x ;$  $x\to1, \Delta x>\Delta y$）的函数，比如 $\sqrt[\alpha]{|x|}$、$1−(|𝑥|−1)^\alpha$ 等来改变权重的形态。根号型函数对中央区域的权重更加激进，而幂型函数则会牺牲极远处和近处的阴影来让中远处的阴影也保持不错的质量。

我们的深度数据不必改变，只需要改变平面上的权重，由于 Minecraft 的体素游戏特性，我们可以对两坐标分别进行权重：
```glsl
vec2 shadowSquareWarp(vec2 p, float a) {
    return p / mix(vec2(1.0), abs(p), a);
}
```
...
可以看到，同样是 2048x 分辨率绘制 8x8 区块的阴影，效果立竿见影：
...
> 
> 另一种在单张阴影贴图上处理的方法叫**直线纹理形变**（**R**ectilinear **T**exture **W**raping）阴影，它会根据视口中阴影的占比动态调整它们在阴影空间中的比例，可以仅使用低分辨率的阴影纹理就达到屏幕像素级别的阴影细节。

### 柔和阴影

虽然近处的阴影细节得到了细化，但是阴影边缘看起来却仍然很粗糙，有许多毛刺和锯齿，这也是阴影纹理与屏幕像素无法精确对齐所致。

虽然我们无法完全避免锯齿的瑕疵，但可以设法遮掩：将阴影的边缘进行柔化。如果我们在一个阴影纹素的周围进行多次取样来判断当前片元被遮蔽的量，就可以求到阴影边缘的过渡，这就是所谓**百分比渐进过滤**（**P**ercentage **C**loser **F**ilter）。

在阴影纹理上进行采样不能像之前的描边模糊着色器那样直接采样纹素夹缝来节省采样次数，因为采样的深度只是几何信息，需要作比较之后才会确定是否遮蔽，平均深度只会让本来就会被遮蔽的纹素依然被遮蔽（不考虑阴影偏移的情况下）。
...
> 仅使用一层花括号来编写这种前层无需额外处理的嵌套循环只是为了排版，不是一个好习惯，初学者一定要规范语法使用：
> 
> ```glsl
> for() {
>     for() {
...
> ```
> 
> ~~否则我会偷偷在你的第一层 `for()` 后面加分号（~~
> 
{style="warning"}

现在，我们需要确定每次采样的位置，采样的方向很简单，就是 `normalize(vec2(i,j))`，我们主要看每一步应该在方向上走多远。最首先的，当然我们需要以纹素为采样尺度，这里的步进坐标是索引坐标，因此我们要将它乘上阴影贴图的纹素大小：
```glsl
const float shadowTexelSize = 1.0 / float(shadowMapResolution);
vec2 steps = vec2(i,j) * shadowTexelSize;
```
然后，我们期望 PCF 只处理阴影的边缘，而不会希望它扩散得太远 ^**1**^。除了除以阴影纹素大小来将采样位置进行收束以外，我们还可以使用另一个值来控制最大扩散距离：
```glsl
[... Settings ...]
#define PCF_RADIUS 1.0
[... 阴影循环 ...]
steps = steps / float(PCF_SAMPLES) * PCF_RADIUS;
```
...
> 在浮点量与整型量进行计算时，一定要保持显式转换类型的好习惯来避免结果中的小数被意外丢弃！
> 
{style="note"}

> 随着阴影纹理分辨率增大，PCF 会变得越来越微弱，如果你希望阴影纹理分辨率与之解耦，可以提前除以一个你测试得恰到好处的参数定值，比如 `2048.0`，而不是乘以 `shadowTexelSize`。
> 
> 如果将阴影的渲染半径增大，PCF 的范围也会增大，如果你希望与阴影渲染半径也解耦，可以额外除以阴影渲染距离 `shadowDistance`，但是需要在之后额外乘一个固定系数，比如测试环境中当前的阴影渲染距离参数 `128.0`。
> ```glsl
> steps = vec2(i,j) / 2048.0 / shadowDistance * 128.0;
> steps = steps / float(PCF_SAMPLES) * PCF_RADIUS;
> ```

由于我们在阴影 Pass 中将阴影空间进行了形变，阴影纹理上的坐标现在是非线性的，这就导致每次采样的坐标变换也需要在采样循环中实时计算，以保证阴影纹理中采样到的内容均匀准确。因此，我们需要未形变阴影空间 NDC 来配合线性的采样步进求得采样点对应的 NDC，然后再将采样点 NDC 进行形变，最后才转换到和阴影纹理对应的形变阴影屏幕空间。

刚才的总步进 *方向* ^**2**^ 是基于未形变阴影屏幕空间的，而我们目前所使用的采样坐标还在 NDC 中，因此为确保扩散正确，需要将步进长度进行对应的映射，也就是翻倍，然而这会让阴影看起来过于模糊，因此我们反而会选择将其**减半**：
```glsl
vec2 samplingNdc = shadowNdcPos.xy + steps * 0.5;
```
**[2]** 还记得之前空间变换的内容吗？**方向不需要进行位移**，即使这个方向包含距离信息。

在进行更多操作之前，我们可以直接在这里进行边界检查，只需要简单地把 NDC 映射到阴影屏幕空间坐标上：
```glsl
if(maxComponent(abs(samplingNdc)) > 1.0) continue;
```
...
```
此外，由于片元的真实深度值是唯一的，因此我们可以直接将当前深度设置在循环外来减少计算量：
```glsl
[... 循环外 ...]
...
在进行深度比较之前，你可能已经考虑到了一个问题，如果平面朝向与光源方向夹角过大时，采样周围坐标上的深度信息可能导致结果和当前着色点上区别也过大，从而造成错误遮蔽，而且采样点距离着色点越远，这个可能产生的深度差就越大。因此我们可以根据采样距离放缩偏移量：
```glsl
[... 循环外偏移处理 ...]
const float bias = ...;
float t = ...;
float biasDir = ...;
[... 循环内偏移处理 ...]
float biasMul = length(steps) * shadowMapResolution + 1.0;
float biasScaled = biasDir * biasMul;
```
我们乘以了 `shadowMapResolution` 来将 `steps` 转换回纹素尺度，当然，你也可以在之前处理采样 NDC 时乘以 `shadowTexelSize` 来避免一次额外乘法：
```glsl
vec2 steps = vec2(i,j) * float(PCF_RADIUS) / float(PCF_SAMPLES) * 0.5;
vec2 samplingNdc = ndc.xy + steps * shadowTexelSize;
float biasScaled = biasDir * (length(steps) + 1.0);
```
最后，像之前那样进行深度比较，然后将结果累积即可：
```glsl
[... 循环内累加 ...]
totalShadow += step(currentDepth - biasScaled, closestDepth);
++sampleCount;
[... 循环外平均 ...]
float shadow = totalShadow / float(sampleCount);
```

![PCF阴影](advancedLighting_pcf_shadow.png){width="700"}

我们将 PCF 的采样质量设置为了 3，用了 $(3_\text{采样质量}\times2_\text{正负两方向}+1_\text{中央})^{2_\text{二维}} = 49$ 次采样获得了比较满意的成果。如此多的采样就昭示着它的性能不会太好，特别是在低端机器上。不过比较方便的是，在我们的算法中，把 `PCF_SAMPLES` 设置为 0 即可禁用 PCF。

## 伽马校正

现在，我们的光照优化已经接近尾声了，在结束这一节之前，让我们再来干一个比较轻松的事情。虽然说是比较轻松，但是由于网络上的各种文章互相摘抄，而作为源头之一的 Learn OpenGL 可以说是完全没解释清楚，导致大家在理解其原理时会出现很多混乱，本小节将致力于用最易懂的文字解释这一过程。

在老式显像管（或者说阴极射线管，**C**athode **R**ay **T**ube）显示设备中，由于显像管的物理特性，导致其输入电压 $U$ 与输出亮度 $L$ 存在幂次关系 ^**1**^ ，具体来说，是 $L\propto U^\gamma, \gamma \in [2.35,2.5]$。这就导致了在受限的电压和显示范围 $[0,1]$ 内，所有亮度都会变暗。

**[1]** 这里所谓的幂次关系不是绝对的电压与亮度的比值，而是在给定黑白电平以及对应的输出电压并归一化的情况下两者之间的关系。

为了纠正这一点，聪明的古人想出了在存储色彩 $C$ 时预先将原始色彩 $C_\text{Raw}$ 进行逆变换，即 $C=C_\text{Raw}^{1/\gamma}$。这样，当存储色彩 $C$ 需要显示在屏幕上时，最终呈现的图像就会经过下式的变换：
$$
C_\text{CRT} = C^{\gamma} = \left(C_\text{Raw}^{1/\gamma}\right)^{\gamma} = C_\text{Raw}
$$

原本保存的线性亮度 _理论上_ 就被完美还原了，不过由于存储时的精度限制，信息的丢失是在所难免的，而这样转换的一个副作用又反而解决了它。

另一个我们 [小学二年级就学过的知识](https://b23.tv/BV1VrVSz1Eme) 是，我们对世界的感知是对数的，古人类为了在黑夜中生存，眼睛对弱光的变化感知更是明显大于强光（韦伯-费希纳定律）。而我们将线性连续的物理光强 $C_\text{Raw}$ 映射到存储色彩 $C$ 之后，还会将数据离散化（比如八位位图的 256 个色阶）。

如果你没意识到这意味着什么，当我们使用小数次幂进行映射时，$[0,1]$ 上会产生上凸的曲线，即映射后的整体值会偏大，但起点和终点不变。也就意味着，小的 $x$ 值就会占据映射后 $y$ 的大部分值（和之前我们所说的阴影中央权重函数类似）。

如果将映射后的曲线在 $y$ 轴上进行均匀离散化，那么最终靠近 $y=0$ 的一端上就会有更加密集的离散值。我们同样 [在 GeoGebra 中创建了一个演示](https://www.geogebra.org/calculator/cah2uccg) 来帮助你更好地理解它：

- $g(x)$ 表示原始连续数据映射到存储数据之后的曲线；
- $p(x)$ 存储数据离散化之后每个灰阶表达的原始数据范围，横线越短，说明原始数据被压缩到这个灰阶上的范围越小，细节就更好；
- $h(x)$ 是最终的灰阶密度函数，表示显示器输出之后每个灰阶覆盖的线性亮度范围，横线长短的表示与 $p(x)$ 一致；
- 你可以调整灰阶数量来观察每个灰阶所表达的原始信息范围、更改伽马值来查看不同的校正曲线如何让每个灰阶的压缩量变更，现代显示器常用的灰阶为 256 个（8bit，0 ~ 255）或 1024 个（10bit，0 ~ 1023），为了让阶跃更加明显，我们将其削减到了 16 ~ 64 个。

也就是说，我们将原始亮度转换到存储亮度时，由于离散化存储在映射之后发生，暗部的信息也歪打正着地得到了更多保留，而我们人眼对这些信息又异常敏感，两边一拍即合，这个过程就被定了下来：
$$
\begin{aligned}
&\text{线性颜色} \xrightarrow{C = C_\text{Raw}^{1/\gamma}} 存储色彩 \to 显示器输出 \\
&\xrightarrow{C_\text{CRT} = C^{\gamma} = C_\text{Raw}} 线性亮度 \to 人眼 \xrightarrow{\log{C_\text{Raw}}} 感知
\end{aligned}
$$
我们现在管这些有特定颜色分布曲线的颜色值集合叫做**色彩空间**（Color Space），而那个作为中间人的存储用非线性色彩空间，现在被我们称为 **sRGB**，将线性亮度转换到它的映射过程被称为**伽马映射**（**Gamma Mapping**），只不过由于各种因缘巧合 ^**2**^，伽马值被统一为了 2.2。由于暗部灰阶保留的特性，现代显示器虽然没了物理伽马特性，却依然会模拟这个过程。

**[2]** 大概是为了避免存储精度不足、通过系统伽马（$编码伽马 \times 显示伽马$）调整感知对比度、顺便平衡一下中间调灰阶之类导致的 ~~，总之定下来 2.2 就对了~~。
...
```
这对于大多数常规颜色来说没有什么大不了，因为它们自始至终都在 sRGB 空间中运转。然而光照之类的物理量应当是在线性空间中的，由于我们没有进行手动处理，线性的光照被以 2.2 次幂输出了，也就是说我们目前所编写的画面实际上是光照不准确 ^**3**^ 的！

**[3]** 在我们目前的创作中这还不是什么大问题，因为我们手动控制了光照强度，使其保持在 $[0,1]$ 这个 SDR 亮度范围附近，顶多会让场景变暗一些。但是到了 HDR 渲染中，情况就完全不同了：由于幂次的存在，那些大于 1 的亮度会被轻易地进行翻倍，越亮的光照会带来越夸张的翻倍，最后导致整个世界的光照强度完全失控。因此进行伽马校正也是为今后的创作铺路。
...
这种将其它色彩空间的颜色通过类似伽马映射的行为以回退到线性空间进行计算的过程，就被称为**伽马校正**（Gamma Correction）。

大多数效果在线性空间中处理都会更好，统一颜色空间本身还更方便，再加上如果你还没忘的上面的光照小节中，我们编写的光照与纹理颜色是耦合的，因此我们没有理由不选第二条路。

虽然很不想承认，但是将所有输入颜色都转换到线性空间是很麻烦的（主要是操作密集型修改，没什么技术含量），包括我们之前使用的 `skyColor` 等等的都是调色之后显示器输出而定下的，因此我们需要留意所有已经用到了的 sRGB 纹理和颜色。但是有一点，Alpha 值通常用于保存其他信息（就像法线和高光贴图那样），它们在创作时通常会刻意设置为特定值，并且基本不参与显示，因此不必进行校正。
...
如果你实在厌烦了 `pow(vec, gvec(float))` 地重复，可以像这样重载一串 `vpow()` 函数：
```glsl
...
vec2 vpow(vec2 a, int b) { return pow(a, ivec2(b)); }
vec3 vpow(vec3 a, int b) { return pow(a, ivec3(b)); }
vec4 vpow(vec4 a, int b) { return pow(a, ivec4(b)); }
```
{collapsible="true" default-state="collapsed" collapsed-title="gvec vpow(vec v, gtype f)"}

然后丢进 `Utilities.glsl` 里，以便随时使用。

最后，我们在 Final 中使用了 `skyColor`，由于它参与了光照计算，因此也要过一遍伽马校正：
```glsl
vec3 skyColorG = vpow(skyColor, GAMMA);
```
> 今后会有更多时候用到这些大气相关的伽马校正颜色，因此你也可以将它和即将使用的 `fogColorG` 一同存放在即将到来的 `Atmosphere.glsl` 中。
> 
{style="note"}
...
2. 将顶点法线进行编码，与光照贴图存放在同一个缓冲区中，由于编码后的法线数据进行了归一化处理，因此你可以放心继续使用归一化类型的缓冲区。
   - 为了和表面法线做区分，今后取出来并解码之后的数据可以单独存放为 `vec3 vertexNormal` 中。
   - 合并完成之后， 2 号缓冲区会空出来（原本用来保存原版光照强度），你可以放心将它留在原处，只更改 `DRAWBUFFERS` 序列和 `location` 索引，而不需要像强迫症那样 *空的缓冲区一定要在后面✍️✍️✍️*。
3. GLSL 中没有内建的 `remap(a,b,x)`、`saturate(x)` 和 `remapSaturate(a,b,x)` 函数，因此你可以将它添加到你的 Utilities 里。
   - 注意 `remapSaturate(a,b,x)` 的内部实现为 `saturate(remap(a,b,x))`，不要写反了！
   - 这种通用函数进行类型重构非常麻烦，因此你可以直接使用宏定义来处理它们。
...
7. 剔除金属的漫反射，让其看起来更加“金属”，使用专用的漫反射强度 `diffuse` 替换之前直接使用的 `albedo.rgb`。
   - 如果 Specular 的 Green 通道表征金属性，那么可以按照其强度来进行处理，同时不要忘记硬编码区域表示完全金属：
     ```glsl
     vec3 diffuse = mix(albedo.rgb, vec3(0.0), material.g);
