...
要想解决这个问题，办法无非其二：
1. 将光照单独提出来计算之后塞进 sRGB 空间
    ```mermaid
    flowchart LR
        A["sRGB 颜色"] --> B["着色器处理"] -- 原样输出 --> C["显示器输出"] --> D["线性颜色"]
        E["线性光照"] --> F["着色器处理"] -- 映射到 sRGB --> C
    ```
2. 或者将所有颜色统统塞进线性空间，然后在输出之前转回到 sRGB。
    ```mermaid
    flowchart LR
        A["sRGB 颜色"] --> E["线性映射"] --> B["着色器处理"] -- "`映射到 sRGB`" --> C["显示器输出"] --> D["线性颜色"]
        F["线性光照"] --> B
    ```

大多数效果在线性空间中处理的效果都会更好，统一颜色空间本身还更方便，再加上如果你还没忘的上面的光照小节，我们编写的光照与纹理颜色是耦合的，因此我们没有理由不选第二条路。
...
好了，至此，我们的延迟处理探索的第一阶段就正式完结了，希望你还没有绕晕，在下一个阶段中，我们将开始着手处理环境，并让我们今天最后所做的伽马校正发挥它的用处。
