...
在本章的第一节中，我们初步体验了延迟渲染，并且留下了很多疑问：奇怪非线性深度图、`gl_FragCoord` 的来由等等，它们都和空间变换直接挂钩。

从这一节开始，我们就将正式进入几何缓冲程序，随着对几何缓冲的深入，上面的问题都将得到解答。
...
由于 GL 使用的**右手坐标系** <sup>**1**</sup>，视口空间的 $z$ 值实际上都是负数，即摄像机始终面朝 $z-$ 方向，距离视平面越远 $z$ 值越小。由于深度需要随距离增加而增大，因此场景的 $z$ 值需要被翻转 <sup>**2**</sup> ，GL 在投影矩阵中执行这一步。
...
> 你可以在 [这个演示](https://www.geogebra.org/calculator/pa7jejre) 中尝试拖动 $n$ 和 $f$ 的值来观察函数 ${z_{\text{NDC}}}(x)$ 在 $n \leq x \leq f$ 上的变化。其中 $X$ 轴表示 $w_{\text{Clip}}$ ，$Y$ 轴表示映射后的 NDC 坐标。
...
2. 尝试调整光照的分量从而改变方向，如果你不确定是否为单位向量，可以使用 `normalize()` 函数将它们转化为标准向量，我们会在下一节中了解这个函数。
