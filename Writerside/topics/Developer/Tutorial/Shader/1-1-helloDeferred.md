...
首先我们需要 GLSL 的另一个内建变量 `gl_FragCoord` ，它指定了当前像素在窗口内的空间坐标。由于我们只关注像素在屏幕上的二维位置，因此我们只关心它的 $xy$ 分量。`gl_FragCoord.xy` 表示了当前像素在横轴和纵轴上的序号（我们可以将其称之为整数坐标或绝对坐标），因此它会随着分辨率的变化而变化。
...
> 
> 这里所谓的 `texture()` 是指“告诉采样器（`sampler2D colortex0`），我们需要对应纹理单元 ID 的缓冲区（colortex0 → 0 号缓冲区）上特定位置（`uv`）的纹理像素”这样的一个行为，目的是**获取纹理像素**。
> 
...
> 如果你想要使用绝对坐标和 `texture()` 函数，可以使用 `sampler2DRect` 类型。
...
好了，在这一节中，你已经学会了着色器中最重要的纹理采样操作，并且封装了几个好用的函数，还认识了一下宏定义。在下一节中，我们将会整理文件结构并引入光影配置，以备迎接几何缓冲。
