...
众所周知，每个语言都会从 Hello World 程序开始，对于 GLSL 来说，它的 Hello World 无疑就是输出一些颜色。那么，让我们把输出变量随意设置一个颜色来看看效果：
...
按下 <shortcut>F3</shortcut><shortcut>R</shortcut> 重载光影，然后，你就已经可以在游戏窗口中看到光影的效果了：

![画一些红色](deferred_drawRed.webp){width="700"}

哇哦！真的是好……呃，纯正的红啊！你会发现整个窗口都染成了红色，就像之前所说的，片元着色器针对屏幕上所有片元执行，因此 `fragColor = vec4(1.0, 0.0, 0.0, 1.0);` 这行代码就意味着往屏幕上所有片元，在这里就是屏幕上的每个像素，都写入了 $\text{RGBA}=(255,0,0,255)$ 的色值，即红色不透明。

当然，只靠这种固定纯色是没法画出场景的，我们需要更多数据来让每个片元根据它们所包含的信息差异来产生不同的颜色，比如像素的**位置**信息本身，以及由不同色值通过规律排布而产生的**图像**。

没错！我们只需要知道像素的位置，然后就能根据它们在窗口中的位置，把其他图像对应位置（一般是相同位置或通过函数映射到不同位置）的像素信息一一对应地取过来了！

## 采样纹理

在 GLSL 中，我们通过**采样**（Sampling）来获取图像的颜色数据，每次采样可以获取图像对应坐标上的颜色，或者说至多四个通道（RGBA）的数据。
...
几何缓冲输出了内容的缓冲区充当了我们采样要用的纹理，即缓冲区是几何缓冲向延迟处理传递信息的媒介：

$$
几何缓冲 \xrightarrow{原始数据} 缓冲区 \xrightarrow{缓存数据} 延迟处理
$$

每个缓冲区的默认尺寸都和屏幕分辨率相当，OptiFine 提供了至多 16 个二维缓冲区供我们使用。由于我们没有自己编写几何缓冲，内置管线默认使用向前渲染法将所有场景输出到了 **0 号缓冲区** `colortex0` 。

现在，我们需要从这个缓冲区中将绘制的场景取出来，即每个像素都需要采样自己坐标对应的缓冲区坐标上的颜色值，然后输出就好。

著名哲学家 _维吉尔_ 曾经说过：“_If you want it, then you'll have to take it._”，如果我们需要采样它，那我们就得先声明它。在 GLSL 中，我们使用**采样器**（sampler）类型来声明缓冲区：
```glsl
...
还记得片元着色器是如何执行的吗？它在每个图元覆盖的每个像素上运行一次，然后将处理结果写入指定缓冲区或屏幕的对应坐标。那么，我们要怎样才能知道当前像素的坐标以采样正确的位置呢？

幸运的是，我们有办法，而且还不止一种。第一种办法，我们直接利用 OptiFine 提供的顶点属性 `vaUV0` ^**1**^，它提供了当前顶点对应的纹理坐标。我们只需要在顶点着色器中传入这个变量，然后赋给传出到片元着色器的变量即可：
```glsl
[...]
in vec2 vaUV0;
out vec2 uv;
...
顶点数据默认会在输出到片元着色器时进行插值，纹理坐标自然就平滑了。接着，我们在片元着色器中传入同名变量：
```glsl
in vec2 uv;
```
...
现在来回想一下，铺屏四边形完全覆盖了屏幕，也就意味着它的纹理坐标和屏幕坐标是一一对应的，因此这也引出了另一种办法，而且可以直接在片元着色器中完成。
...
**[2]** 像素从左下角开始的横纵方向上的序号（从 $(0,0)$ 开始的索引）组成的坐标。
...
**[1]** 在 OpenGL 中，带有 _`g`_ 前缀的类型表示有多种可选子类型，通常包括整型前缀 `i`，无符号前缀 `u` 和布尔前缀 `b`，默认无前缀的则通常是浮点类型。此外，这里还省略了类型的后缀，部分类型有多种后缀，通常是表示维度，比如采样器的子类型 `2D`、`3D`、`Cube` 等和向量的维度 `2`、`3`、`4`。

现在，让我们修改片元着色器，试着向屏幕上输出场景：
```glsl
#version 330 core
...
缓冲区的默认行为是 `GL_CLAMP_TO_EDGE` 。然而，在 OptiFine 管线中，我们无法控制每个纹理的边缘行为，因此约束坐标就显得很重要了。我们可以手动实现上述的四种边缘行为，同时附上将画面缩小到纵横各 1/4 后（**提前**将 `uv` 扩大 4 倍，用于演示大于 `1.0` 的坐标处置方式）的处理结果：
...
如果你这样想了，那么恭喜你，你已经悟出了着色器中非常重要，也是很多现代特效极其依赖的采样手法：多次采样。一提到它，我们最容易想到的就是**模糊**，因为它的核心思想就是将像素的颜色**扩散**到周围，而在片元着色器中，我们可以逆向操作，将周围的像素扩散过来。
