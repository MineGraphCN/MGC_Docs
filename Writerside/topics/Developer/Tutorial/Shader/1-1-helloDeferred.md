...
哇哦！真的是好……呃，纯正的红啊！你会发现整个窗口都染成了红色，就像之前所说的，这是因为我们的片元着色器是针对屏幕上所有片元执行的。当然，只靠这种固定纯色是没法画出场景的，我们需要更多数据来让每个片元根据它们所包含的信息差异来产生不同的颜色。具体点说，会基于像素坐标变化的数据……比如像素的**位置**，以及**图像**。

没错！我们只需要知道像素的位置，然后就能根据它们在窗口中的位置，把其他图像相同位置（或者通过函数映射到不同位置）的像素信息一一对应地取过来了！
