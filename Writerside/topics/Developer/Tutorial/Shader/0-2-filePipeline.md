...
> 较新的 Iris 针对嵌套文件夹的情况做了兼容，因此无需检查多重嵌套的情况。但是为了兼容性，在编写和打包时最好还是让文件结构符合要求。
...
回到 `shaders` 文件夹下，OptiFine 在检查了所有默认读取的文件和文件夹之后，开始编译光影。它会将根文件夹和每个维度文件夹下着色器文件的代码编译为程序。根据管线顺序，OptiFine 会主动读取并编译 `.vsh`、`.gsh`、`.csh` 和 `.fsh` 文件，分别对应 *顶点着色器*、*几何着色器*、*计算着色器* 和 *片元着色器*。
...
![OptiFine 管线](ofpp_simple.webp "OptiFine 管线"){width="700"}
