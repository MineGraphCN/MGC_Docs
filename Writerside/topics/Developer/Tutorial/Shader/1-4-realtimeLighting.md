...
```glsl
uniform sampler2D lightmap; // 在几何缓冲可用的光照贴图。
```
```glsl
in ivec2 vaUV2; // 光照纹理坐标
```
...
现在还剩下一个 _小_ 问题：我们采样的光照颜色是天空和方块光照的叠加，而我们之前写的光照强度是根据太阳的方向变化：我们没有理由让方块光照强度也随着太阳的变化而变化！

其中一个解决办法是在几何缓冲中处理一次表面光照，然后根据光照的方向动态地变化我们在光照贴图上的采样纵坐标。但是这种方法比较繁琐，并没有多少光影使用，相反，我们通常只保存纹理坐标作为光照强度，然后自己处理光照色彩。

虽然在下一节中绘制实时阴影之后，我们不必再依赖光照贴图遮挡阳光或月光，但是天空光照强度数据仍可用于环境光或另作他用。我们会将输出光照强度留作习题。

> 事实上，`lightmap` 是随着时间动态变化的，天空光照有一个随着遮挡变化的固有强度和一个基于时间的系数，刷怪判定就由它们共同完成，这个系数也会改变不同时间段光照贴图的颜色，从而营造出光照随时间的变化。
>
> 如果成功直接输出了光照强度你就会发现，游戏中天空光照随时间变化的时候光照贴图的纹理坐标其实并没有变化，因此只能是 `lightmap` 中的内容变化。
> 
...
现在好多了，但是你会发现场景出现了一些悬空，这是不可避免的。在极端情况下这个偏移值可能过小，你当然可以直接调大它，但是一个更明智的方法是根据表面的法线动态地调整偏移量。
...
将 `vaUV2` 处理之后传入像素着色器，将光照强度独立拆分到两个通道中输出，不要使用 `lightmap` ，然后在 `final.fsh` 中仅将天空光照强度乘以环境光强度，并在最终光照强度上独立叠加方块光照强度。注意：
