...
因此，Mojang 的思路是只在每个顶点做一次光照颜色采样，然后把剩下的活都交给顶点插值完成。这种方法只需要在每个顶点而不是每个片段执行一次采样，而且可以保证一个方块上的光照过渡均匀，我们没什么理由不仿效。
...
```
不过我们目前不需要向它们输出内容，因为我们也有专用的深度缓冲区 `shadowtex0` 和 `shadowtex1` 。和 `depthtex` 类似，`0` 中包含了半透明几何的深度。我们只需要在片段着色器中将深度数据（希望你还记得是 `gl_FragCoord.z`）写入 `gl_FragDepth` 就行了，就像 GL 默认的那样。但是还有一件事：记得做 Alpha 测试，不然场景中本应透明的植被等区域也会被覆盖！这也是我们在顶点着色器程序传出 `uv` 的原因。
```glsl
#version 330 core
