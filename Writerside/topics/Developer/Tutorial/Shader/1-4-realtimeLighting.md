...
在原版中，场景随着时间和位置改变天空光照亮度（或者说阳光/月光亮度）的行为和光源方块的照明行为均由光照贴图（Light Map）完成，它们表征了方块受到的光照强度，由游戏（于 **JE 1.21.2 24w33a** 作为可访问的着色器 `lightmap` 加入资源包）动态生成。OptiFine 为我们提供了相关数据：
```glsl
...
> 
> 由于 **JE 1.21.9 25w31a** 大幅优化了实体渲染，如今不再会逐个绘制实体，而是将状态相同的成批绘制，同批的几何体只有一次绘制请求。也就是说统一变量无法在这个过程中被修改，因此，利用统一变量绘制实体叠加色可能会称为过去式。
...
```
即缩小 $16 \times 16 = 256$ 倍再位移半个纹素（$0.03125 \times 16 = 0.5$）以在中央取 $[0, 15]$ 共 16 级光照强度对应的颜色值，和我们直接除以 16 再用索引坐标进行纹素取样的方法是类似的。只不过我们的方法不会进行纹素插值，但是我们会在顶点着色器中采样，每个顶点本来也就只有一个颜色值，因此无所谓。但是这也引出了下一个问题：在顶点着色器中采样纹理之后会发生什么？
