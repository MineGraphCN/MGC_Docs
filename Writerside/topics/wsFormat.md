...
> 在章节后加空格并接 `{id="xxx"}` 可以设置该章节的跳转 ID，可以在链接中使用。
> 
> 标题中的西文字符会自动设置为跳转 ID，若没有西文则设置为编译之后的 UUID，每次编译都会变化。
> 
> 手动设置的 ID 无西文要求，可以任意设置，仅当你会在其他地方（无论是站内还是站外）跳转到这个位置时才为它们设置 ID。
> 
{style="note"}
...
> 
> 几乎所有 XML 块都可以像标题那样设置 ID，但是跳转位置不一定精确。
...
>   ```
> - `type` 字段见 `<list>` 块
> - 使用 <shortcut>Tab</shortcut> 缩进可以将其他内容悬挂到当前列表下。
...
> `type` ：`bullet` (默认) 点号，`decimal` 同有序列表，`alpha-lower` 使用小写字母的有序列表，`none` 没有前缀。
> 
...
- 使用连续的三根横线 `---` 可以隔断 markdown 块。
- 在任意 markdown 块下空一行（如果对应块下会强制换行则不需要再空一行，比如表格）再使用 `{}` 可以定义对应 xml 块的属性。例：
  <procedure>
